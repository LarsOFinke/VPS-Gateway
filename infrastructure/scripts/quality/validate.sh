...
mapfile -t shell_files < <(find . -type f \( -name '*.sh' -o -path './scripts/connect-route' \
  -o -path './scripts/remove-route' -o -path './scripts/list-routes' -o -path './scripts/setup' \
  -o -path './scripts/renew-certificates' \))
bash -n "${shell_files[@]}"
